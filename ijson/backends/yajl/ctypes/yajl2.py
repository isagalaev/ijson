...
def yajl_init(scope, events, allow_comments=False, multiple_values=False):
    ...
    handle = yajl.yajl_alloc(byref(scope.callbacks), None, None)
    if allow_comments:
...
def basic_parse(f, buf_size=64*1024, allow_comments=False, multiple_values=False):
    _yajl = sys.modules[__name__]
    return ijson.backends.yajl.common.basic_parse(
        _yajl, f, buf_size=buf_size,
        allow_comments=allow_comments, multiple_values=multiple_values
    )
